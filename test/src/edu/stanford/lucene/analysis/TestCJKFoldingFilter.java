...
//		checkOneTerm(analyzer, "侮", "侮"); // 侮 4FAE => (侮) FA30
		checkOneTerm(analyzer, "倹", "儉"); // 倹 5039 => (儉) 5109
		checkOneTerm(analyzer, "偽", "僞"); // 偽 507D => (僞) 50DE
//		checkOneTerm(analyzer, "免", "免"); // 免 514D => (免) FA32
...
//		checkOneTerm(analyzer, "勤", "勤"); // 勤 52E4 => (勤) FA34
...
//		checkOneTerm(analyzer, "勉", "勉"); // 勉 52C9 => (勉) FA33
		checkOneTerm(analyzer, "勲", "勳"); // 勲 52F2 => (勳) 52F3
//		checkOneTerm(analyzer, "卑", "卑"); // 卑 5351 => (卑) FA35
...
//		checkOneTerm(analyzer, "嘆", "嘆"); // 嘆 5606 => (嘆) FA37
//		checkOneTerm(analyzer, "器", "器"); // 器 5668 => (器) FA38
...
//		checkOneTerm(analyzer, "塀", "塀"); // 塀 5840 => (塀) FA39
		checkOneTerm(analyzer, "塁", "壘"); // 塁 5841 => (壘) 58D8
//		checkOneTerm(analyzer, "塚", "塚"); // 塚 585A => (塚) FA10
...
//		checkOneTerm(analyzer, "墨", "墨"); // 墨 58A8 => (墨) FA3A
		checkOneTerm(analyzer, "壊", "壞"); // 壊 58CA => (壞) 58DE
...
//		checkOneTerm(analyzer, "層", "層"); // 層 5C64 => (層) FA3B
...
//		checkOneTerm(analyzer, "廊", "廊"); // 廊 5ECA => (廊) F928
...
//		checkOneTerm(analyzer, "慨", "慨"); // 慨 6168 => (慨) FA3E
//		checkOneTerm(analyzer, "憎", "憎"); // 憎 618E => (憎) FA3F
		checkOneTerm(analyzer, "懐", "懷"); // 懐 61D0 => (懷) 61F7
//		checkOneTerm(analyzer, "懲", "懲"); // 懲 61F2 => (懲) FA40
...
//		checkOneTerm(analyzer, "敏", "敏"); // 敏 654F => (敏) FA41
...
//		checkOneTerm(analyzer, "暑", "暑"); // 暑 6691 => (暑) FA43
		checkOneTerm(analyzer, "暦", "曆"); // 暦 66A6 => (曆) 66C6
//		checkOneTerm(analyzer, "朗", "朗"); // 朗 6717 => (朗) F929
...
//		checkOneTerm(analyzer, "梅", "梅"); // 梅 6885 => (梅) FA44
...
//		checkOneTerm(analyzer, "海", "海"); // 海 6D77 => (海) FA45
		checkOneTerm(analyzer, "渇", "渴"); // 渇 6E07 => (渴) 6E34
		checkOneTerm(analyzer, "温", "溫"); // 温 6E29 => (溫) 6EAB
//		checkOneTerm(analyzer, "漢", "漢"); // 漢 6F22 => (漢) FA47
		checkOneTerm(analyzer, "権", "權"); // 権 6A29 => (權) 6B0A
		checkOneTerm(analyzer, "横", "橫"); // 横 6A2A =>（橫） 6A6B
//		checkOneTerm(analyzer, "欄", "欄"); // 欄 6B04 => (欄) F91D
...
//		checkOneTerm(analyzer, "殺", "殺"); // 殺 6BBA => (殺) F970
		checkOneTerm(analyzer, "殻", "殼"); // 殻 6BBB =>（殼） 6BBC
...
//		checkOneTerm(analyzer, "煮", "煮"); // 煮 716E => (煮) FA48
...
//		checkOneTerm(analyzer, "碑", "碑"); // 碑 7891 => (碑) FA4B
//		checkOneTerm(analyzer, "祉", "祉"); // 祉 7949 => (祉) FA4D
//		checkOneTerm(analyzer, "祥", "祥"); // 祥 7965 => (祥) FA1A
		checkOneTerm(analyzer, "禍", "禍"); // 禍 798D => (禍) FA52
//		checkOneTerm(analyzer, "福", "福"); // 福 798F => (福) FA1B
...
//		checkOneTerm(analyzer, "穀", "穀"); // 穀 7A40 => (穀) FA54
		checkOneTerm(analyzer, "穂", "穗"); // 穂 7A42 => (穗) 7A57
//		checkOneTerm(analyzer, "突", "突"); // 突 7A81 => (突) FA55
		checkOneTerm(analyzer, "竜", "龍"); // 竜 7ADC => (龍) 9F8D
//		checkOneTerm(analyzer, "節", "節"); // 節 7BC0 => (節) FA56
...
//		checkOneTerm(analyzer, "諸", "諸"); // 諸 8AF8 => (諸) FA22
		checkOneTerm(analyzer, "謡", "謠"); // 謡 8B21 => (謠) 8B20
//		checkOneTerm(analyzer, "謹", "謹"); // 謹 8B39 => (謹) FA63
		checkOneTerm(analyzer, "譲", "讓"); // 譲 8B72 => (讓) 8B93
		checkOneTerm(analyzer, "豊", "豐"); // 豊 8C4A => (豐) 8C50
//		checkOneTerm(analyzer, "賓", "賓"); // 賓 8CD3 => (賓) FA64
		checkOneTerm(analyzer, "賛", "贊"); // 賛 8CDB => (贊) 8D0A
//		checkOneTerm(analyzer, "贈", "贈"); // 贈 8D08 => (贈) FA65
...
//		checkOneTerm(analyzer, "都", "都"); // 都 90FD => (都) FA26
...
//		checkOneTerm(analyzer, "隆", "隆"); // 隆 9686 => (隆) F9DC
		checkOneTerm(analyzer, "隠", "隱"); // 隠 96A0 => (隱) 96B1
		checkOneTerm(analyzer, "雑", "雜"); // 雑 96D1 => (雜) 96DC
//		checkOneTerm(analyzer, "難", "難"); // 難 96E3 => (難) FA68
		checkOneTerm(analyzer, "霊", "靈"); // 霊 970A => (靈) 9748
//		checkOneTerm(analyzer, "響", "響"); // 響 97FF => (響) FA69
//		checkOneTerm(analyzer, "頻", "頻"); // 頻 983B => (頻) FA6A
		checkOneTerm(analyzer, "頼", "賴"); // 頼 983C => (賴) 8CF4
		checkOneTerm(analyzer, "顕", "顯"); // 顕 9855 => (顯) 986F
//		checkOneTerm(analyzer, "類", "類"); // 類 985E => (類) F9D0
